...
public class GameAPI extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
         ...
         * 5. 把两个游戏玩家加入到游戏房间中
         *
         * 当前逻辑是在 game_room 页面加载的时候进行的, 前面在 创建房间/匹配 的过程中构造的时候没有为 User 赋值.
         *
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        ...
        /**
         * 判定多开的时候(已经是上线状态), 需要 关闭 第二个session的时候会触发此方法并且误删第一个session, 所以,
         * 只有判断两个session一样(只有一个session)的时候才会执行 onlineUserManager.exitGameRoom(user.getUserId()); 方法!
         */
    ...
    private void noticeThatUserWin(User user) throws IOException {
        ...
        Map<String, Object> result = fabricService.queryUserByUsername(thatUser.getUsername());
        if (result == null) {
        ...
        result = fabricService.updateUserStats(
                thatUser.getUsername(),
                winuser.getTotalCount() + 1,
                winuser.getWinCount() + 1,
                winuser.getScore() + 30);
        if (result == null) {
        ...
        result = fabricService.queryUserByUsername(user.getUsername());
        if (result == null) {
        ...
        result = fabricService.updateUserStats(
                user.getUsername(), loseuser.getTotalCount() + 1, loseuser.getWinCount(), loseuser.getScore() - 30);
        if (result == null) {
        ...
        roomManager.removeRoom(
                room.getRoomId(), room.getPlayer1().getUserId(), room.getPlayer2().getUserId());
    }
}
